...
public class View_friends extends AppCompatActivity {
    ListView lv1;
    String[]ulid,uname,uemail,status,uimage,tolid;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String url = "http://" + hu + ":5000/and_my_friend";
                        ...
                        try {
                            ...
                            if (jsonObj.getString("status").equalsIgnoreCase("ok")) {
                                ...
                                for(int i=0;i<js.length();i++)
                                {
                                    ...
                                    ulid[i]=u.getString("frid");
                                    uname[i]=u.getString("name");
                                    uemail[i]=u.getString("email");
                                    uimage[i]=u.getString("photo");
                                    status[i]=u.getString("status");
                                    tolid[i]=u.getString("ulid");


                                }
                                lv1.setAdapter(new Custom_view_friends(getApplicationContext(),ulid,uname,uemail,uimage,status,tolid));
